...
export const validateEmails = (email, errorMessage) => {
  ...
  export const handleResponse = (res, name, successMessage, duplicateAlert, invalidFormat, validateCharacter) => {
    if (res.data && res.data.status === false && res.data.message) {
      ...
      if (messageToShow.includes('format is invalid')) {
        messageToShow = invalidFormat;
      }
      if (messageToShow.includes('must not be greater than')) {
        messageToShow = validateCharacter;
      } else if (messageToShow.includes('has already been taken') || messageToShow.includes('value tried to register') || messageToShow.includes('with the same characteristics already exists') || messageToShow.includes('An activity with the same name and type already exists')) {
